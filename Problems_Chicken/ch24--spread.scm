;; As found in the text but reformatted (mostly to remove tabs).
;; "required.scm" or #lang simply_scheme required for the text book
;; standard environment.

;; Added vim synonyms for movement even though I'll probably use
;; select.
...
;; NOTE: an attempt to restart after an invalid command. I'm not sure
;; how well this will work.

(define (resume)
  ;; should the selection and screen stuff be added?
  (command-loop))


;; NOTE: exit isn't working as I expect. It's probably something to do
;; with my Chicken readline environment. To exit "exit" works, but 'exit,
;; (exit), and exit do not.
...
(define *the-commands*
  (list (list 'p prev-row)
        ...
        (list 'b prev-col)
        (list 'f next-col)
        (list 'h prev-col)       ;; vim equivalents
        (list 'j next-row)       ;; ..
        (list 'k prev-row)       ;; ..
        (list 'l next-col)       ;; ..
        (list 'select select)
...
;; NOTE: let's make 'error' recoverable.
;; 'error' is a standard procedure and these days is expect to halt the
;; program. I'm not sure this can easily be changed but I'm leaving the
;; note as a reminder.
;; ..
;; In testing so far I wrote a 'resume' to restart the screen loop. It
;; seems to be working well enough for my purposes.
