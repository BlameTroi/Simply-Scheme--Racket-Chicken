...
(define (count-nodes-r xs)
  (cond ((empty? xs)  0)
        (else      (+ (count-nodes (car xs)) (count-nodes-r (cdr xs))))))
(define (count-nodes x)
  (cond ((leaf? x)    1)
        (else      (+ 1 (count-nodes-r (cdr x))))))
...
;; ---------- from text for testing comparison ----------
(define (compute tree)
...
(define (parse-helper expr operators operands)
  (cond ((null? expr) (if (null? operators)
        ...
        ((list? (car expr)) (parse-helper (cdr expr)
                                          operators
                                          (cons (parse (car expr)) operands)))
        (else (if (or (null? operators) (> (precedence (car expr)) (precedence (car operators))))
...
(define (parse-scheme-r src op-stack term-stack)
  (cond ((null? src)
         (if (null? op-stack)
           (car term-stack)       ;; error?
           (make-node (car op-stack) (list (cadr term-stack) (car term-stack)))))
        ((number? (car src))
...
;; Test, be sure to get the order on a subtract correct!

(define expr-1 '(* (+ 4 3) 2))
(define ans-1 14)
(define expr-2 '(- 1 2))
(define ans-2 -1)
(define expr-3 '(* 2 (* 2 (* 2 2))))
(define ans-3 16)
