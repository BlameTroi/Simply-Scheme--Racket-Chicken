...
;; ----------------------------------------------
;; 8.3 Describe each of the following functions in English. Include the domain
;; and range of each.

;; This will lead into a discussion of the authors' recursion helpers. I've
;; left those notes in with 8.3.

(define (f a)
  (keep even? a))

;; The above is a selection of all the elements in `a' that are even numbers.
;; The domain of `a' is a sentence (list) of only numbers. The range of `f' is
;; all the even numbers in 'a'.
...
;; The above returns the result of applying function `b' to each item in the
;; sentence '(blue jay way). The domain of `g' is any function that accepts a
;; single word argument. The range is a sentence of the results of applying `b'
;; to each item.

(define (h c d)
  (c (c d)))

;; The above mess defines function `h' as a function that takes two arguments,
;; both of which must be functions. The first function argument `c' must be a
;; function that accepts a single argument, which itself must be a function
;; that accepts a function as an argument. Functions `h' and `c' both return a
...
;;            (define (h f1a f1b) ...)
;;
;; The domain of `h' is all single argument functions that take functions as
;; arguments and return like functions as arguments.
...
;;
;; Domain ... functions that two arguments of some 'thing' and return something
;; that can be used as an argument of the function. Range ... Any one 'thing'
;; produced by the function.

;; sqrt

;; Compute the square root of a number. Its domain is all numbers, as is its
;; range.

;; repeated

;; `repeated' is a function that takes as arguments a `function' and a numeric
...
;; A function that takes a number (domain of `sqrt') and applies `sqrt' 3 times
;; in succession. The functions domain and range are the same as `sqrt'.
...
;; A function that takes a list (or some type that can be enumerated) and
;; returns the first of the first of the list/enumerable. I expect that ((...)
